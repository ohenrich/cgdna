...
FixNVEDotc::FixNVEDotc(LAMMPS *lmp, int narg, char **arg) :
  FixNVE(lmp, narg, arg) {}

/* ---------------------------------------------------------------------- */

void FixNVEDotc::init()
{
  avec = (AtomVecEllipsoid *) atom->style_match("ellipsoid");
  if (!avec)
    error->all(FLERR,"Compute nve/dotc requires atom style ellipsoid");
  ...
  for (int i = 0; i < nlocal; i++)
    if (mask[i] & groupbit)
      if (ellipsoid[i] < 0)
        error->one(FLERR,"Fix nve/dotc requires extended particles");
...
void FixNVEDotc::initial_integrate(int vflag)
{
  double *shape,*quat;
...
void FixNVEDotc::final_integrate()
{
